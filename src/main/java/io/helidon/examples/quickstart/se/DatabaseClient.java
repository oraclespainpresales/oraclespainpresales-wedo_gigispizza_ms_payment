...
public class DatabaseClient {
	...
	private String executeInsertPayment(String paymentCd, 
		...
		try (Connection conn = getConnectionFromEnvVars();Statement stat = conn.createStatement()) {
			...
			System.out.println("parameter 5 oriPrice: "   + oriPrice);
			System.out.println("parameter 6 totPaid: "    + totPaid);
			System.out.println("parameter 7 custId: "     + custId);
			...
			pstat.setFloat (5,Float.parseFloat(oriPrice));
			pstat.setFloat (6,Float.parseFloat(totPaid));
			pstat.setString(7,custId);
			...
			try (ResultSet rset = stat.executeQuery(query)) {
				...
				while(rset.next()){
					...
					for (int i=1;i <= numColumns;i++){
						//if (!"SERVICESURVEY".equals(metaData.getColumnLabel(i)))		
						String colValue = rset.getString(i);									
						jColumnBuilder.add(metaData.getColumnLabel(i), rset.wasNull()? "no comments" : colValue);
					}
