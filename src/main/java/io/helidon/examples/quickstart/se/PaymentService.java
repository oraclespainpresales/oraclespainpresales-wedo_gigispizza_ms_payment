...
public class PaymentService implements Service {
	...
	public void update(Routing.Rules rules) {
		rules
			.post("/version", this::getAppVersion)
			.post("/selectpayment", this::selectDatabaseHandler)
			.post("/payment", this::updateDatabaseHandler);
	}

	/**
	 * response with microservice num version
	 * @param request
	 * @param response
	 */
	private void getAppVersion (ServerRequest request, ServerResponse response) {
		try{
			System.out.println("Version:" + Main.versionNumber);
			response.headers().contentType(MediaType.APPLICATION_JSON);
			JsonObject returnObject = JSON.createObjectBuilder()
				.add("version", Main.versionNumber)				
				.build();
			response.status(Http.Status.OK_200).send(returnObject);
		}
		catch(Exception ex){
			System.err.println("ERROR version: " + ex.getMessage());
